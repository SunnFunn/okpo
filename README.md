...
4. Пишет Excel `tmp/test_check_YYYY-MM-DD.xlsx`: строки сгруппированы по станциям/дорогам/источнику, последняя колонка — **Количество вагонов** (для `NoNumber` из JSON берётся `CarCount`).
